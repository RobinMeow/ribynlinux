...
# SOURCE: https://unix.stackexchange.com/revisions/480191/12
...
#
# To get maximum read throughput over USB, drop the job count to 1 and reduce the queue depth to 16

# 5 loops is the usual default. but im fine with 2 or even 1
LOOPS=2     #How many times to run each test
...
if [ -z $1 ]; then
	echo "Target required. pass in $HOME as first argument to test your default drive at home dest."
	exit 1
else
	TARGET="$1"
