...
cp "$RIBYN_ROOT/lib/kitty/config/current-theme.conf" "$HOME/.config/kitty/"
...
if [[ ${RIBYN_KITTY_USE_SOLID_BG:-"no"} == "no" ]]; then
	cp "$RIBYN_ROOT/lib/kitty/config/kitty.conf" "$HOME/.config/kitty/"
else
	cp "$RIBYN_ROOT/lib/kitty/kitty-solid-bg.conf" "$HOME/.config/kitty/kitty.conf"
fi
