...
logfile="$HOME/.cache/ribyn/hypr/hyprtoolkit-build-from-source.log"
...
if on_arch; then
	...
	(
		dest="$HOME/.local/share/ribyn/hypr/hyprtoolkit"
		if [[ -d "$dest" ]]; then
			info "hyprtoolkit already installed. skipping."
		else
			info "hyprtoolkit building from source"
			git clone "https://github.com/hyprwm/hyprtoolkit.git" "$dest"
			cd "$dest"
			git checkout $RIBYN_HYPR_HYPRTOOLKIT_GIT_REF
